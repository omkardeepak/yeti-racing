...
export default function SponsorsPage() {
  ...
  useEffect(() => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            entry.target.querySelector('img').classList.add('scale-out');
          } else {
            entry.target.querySelector('img').classList.remove('scale-out');
          }
        });
      },
    ...
    const divElements = divRef.current.querySelectorAll('.observed-div');
    divElements.forEach((div) => observer.observe(div));

    return () => {
      divElements.forEach((div) => observer.unobserve(div));
    };
  }, []);
      ...
      <div className="bg-gradient-to-b from-black to-red-900 pb-20">
        <div className="max-w-[90%] 2xl:max-w-[1400px] mx-auto px-4 py-16">
          ...
          <div className="max-w-6xl mx-auto animate-[slideIn_1s_ease-in-out] translate-x-75">
            ...
            <div className="p-8 rounded-lg observed-div" ref={divRef} >
